...
class FlexmarkMarkdownMapperTest {
  ...
  void whenMarkdownIsNullOrEmpty_thenDocumentIsEmpty(String markdown) {
    ...
    // Then
    assertThat(document)
        .as("Document for null/empty markdown must not be null")
        .isNotNull();
  ...
  void whenMarkdownContainsHeadingAndParagraph_thenAstContainsHeadingAndParagraph() {
    ...
    assertThat(heading.children())
        .singleElement()
        .isInstanceOf(Text.class);
    assertThat(((Text) heading.children().get(0)).text())
        .isEqualTo("Title");
    ...
    assertThat(paragraph.children())
        .singleElement()
        .isInstanceOf(Text.class);
    assertThat(((Text) paragraph.children().get(0)).text())
        .isEqualTo("Some text.");
  }
  ...
  void whenMarkdownContainsBulletList_thenAstContainsBulletListAndItems() {
    ...
    assertThat(firstItem.children())
        .first()
        .isInstanceOf(Paragraph.class);
    Paragraph firstParagraph = (Paragraph) firstItem.children().get(0);
    assertThat(firstParagraph.children())
        .singleElement()
        .isInstanceOf(Text.class);
    assertThat(((Text) firstParagraph.children().get(0)).text())
        .isEqualTo("first item");

    // Second ListItem
    ListItem secondItem = list.items().get(1);
    assertThat(secondItem.children())
        .first()
        .isInstanceOf(Paragraph.class);
    Paragraph secondParagraph = (Paragraph) secondItem.children().get(0);
    assertThat(secondParagraph.children())
        .singleElement()
        .isInstanceOf(Text.class);
    assertThat(((Text) secondParagraph.children().get(0)).text())
  ...
  @DisplayName("When AST is created manually and mapped to markdown and back, then structure stays intact")
  @Test
  void whenAstRoundTripsThroughMarkdown_thenStructureIsPreserved() {
            ...
            List.of(
                ...
                new BulletList(
                    List.of(
                        new ListItem(
                            List.of(
                                new Paragraph(List.of(new Text("first item")))
                            )),
                        new ListItem(
                            List.of(
                                new Paragraph(List.of(new Text("second item")))
                            ))))));
    ...
    assertThat(list.items())
        .hasSize(2);
    assertThat(((Text) ((Paragraph) list.items().get(0).children().get(0)).children().get(0)).text())
        .isEqualTo("first item");
    assertThat(((Text) ((Paragraph) list.items().get(1).children().get(0)).children().get(0)).text())
        .isEqualTo("second item");
  }
}
